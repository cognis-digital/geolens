...
How **geolens** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `geolens` lives in the **Geospatial / GEOINT** cluster.
...
| from | relation | to |
|---|---|---|
| `geolens` | JSON findings compose with | [`locateanything`](https://github.com/cognis-digital/locateanything), [`geoaoi-pro`](https://github.com/cognis-digital/geoaoi-pro) |
| `geolens` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `geolens` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `geolens` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the geospatial / geoint cluster.** `geolens` output feeds the next tool:
```bash
geolens ... --format json > out.json          # this tool's findings (see `geolens --help`)
...
geolens ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
geolens ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
geolens ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `geolens`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `geolens` slots
