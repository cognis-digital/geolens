...
**geolens** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `geolens` emits JSON findings, pipe them straight to a destination — `--dry-run`
...
works whatever `geolens` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="geolens") for rec in records]   # records = your JSON output
