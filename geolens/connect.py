"""Native cognis-connect emit for geolens — forward findings to any platform.

Maps geolens's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    geolens ... --format json | geolens-emit --to stix
    geolens-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['title'] = rec.get('title', 'Geolocation Finding')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'geolocation')
